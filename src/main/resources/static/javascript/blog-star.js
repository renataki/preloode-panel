document.addEventListener("DOMContentLoaded", function(event) {
    ...
    tinymce.init({
        ...
        "convert_urls": true,
        "height": 300,
        "images_upload_url": document.getElementById("global").getAttribute("data-base-url") + "/blog/star/upload-tinymce",
        "image_advtab": true,
...
app.controller("blogStar", ["$scope", "$window", "global", function($scope, $window, global) {
    ...
    $scope.delete = function(id, event) {
        ...
        var rest = {
            "data": id,
            "url": document.getElementById("global").getAttribute("data-base-url") + "/blog/star/delete"
        };
    ...
    $scope.edit = function(event) {
        ...
        if(valid) {
            ...
            var rest = {
                ...
                "url": document.getElementById("global").getAttribute("data-base-url") + "/blog/star/update"
            };
            global.rest(rest, function(response) {
    ...
    $scope.filterPagination = function(event) {
        ...
        var rest = {
            "data": data,
            "url": document.getElementById("global").getAttribute("data-base-url") + "/blog/star/filter-pagination"
        };
        global.rest(rest, function(response) {
    ...
    $scope.forceUploadImageList = function() {
        ...
        document.getElementsByClassName("blog-star-image-list")[0].click();
    ...
    $scope.forceUploadThumbnailList = function() {
        ...
        document.getElementsByClassName("blog-star-thumbnail-list")[0].click();
    ...
    $scope.goToPage = function(event) {
        ...
        if(event.which == 13) {
            ...
            $window.location.href = document.getElementById("global").getAttribute("data-base-url") + "/blog/star/page-" + $scope.site.page.number + "/";
    ...
    $scope.initializeData = function(id) {
        ...
        var rest = {
            "data": id,
            "url": document.getElementById("global").getAttribute("data-base-url") + "/blog/star/initialize-data"
        };
        global.rest(rest, function(response) {

            if(response.result) {

                if(response.blogStar.id != null) {

                    for(var i = 0; i < response.blogStar.category.idList.length; i++) {

                        $scope.category.idList.value.push(response.blogStar.category.idList[i]);
                        $scope.category.nameList.value.push(response.blogStar.category.nameList[i]);
                        $scope.category.pathList.value.push(response.blogStar.category.pathList[i]);
                        $scope.category.urlList.value.push(response.blogStar.category.urlList[i]);

                    }

                    for(var i = 0; i < response.blogStar.company.idList.length; i++) {

                        $scope.company.idList.value.push(response.blogStar.company.idList[i]);
                        $scope.company.nameList.value.push(response.blogStar.company.nameList[i]);

                    }

                    for(var i = 0; i < response.blogStar.company.branch.idList.length; i++) {

                        $scope.company.branch.idList.value.push(response.blogStar.company.branch.idList[i]);
                        $scope.company.branch.nameList.value.push(response.blogStar.company.branch.nameList[i]);

                    }

                    $scope.content.value = response.blogStar.content;

                    var createdTimestamp = new Date(response.blogStar.created.timestamp);
                    ...
                    $scope.created.user.id.value = response.blogStar.created.user.id;
                    $scope.created.user.username.value = response.blogStar.created.user.username;
                    $scope.description.value = response.blogStar.description;
                    $scope.dislike.value = library.numeral.initializeSeparator(response.blogStar.dislike, true);

                    if(response.blogStar.imageList.length > 0) {

                        $scope.imageList.value = response.blogStar.imageList;
                        $scope.imageList.upload.file = response.blogStar.imageList;
                        $scope.imageList.upload.view = true;

                    }

                    $scope.id.value = response.blogStar.id;
                    $scope.like.value = library.numeral.initializeSeparator(response.blogStar.like, true);
                    $scope.meta.description.value = response.blogStar.meta.description;
                    $scope.meta.keyword.value = response.blogStar.meta.keyword;
                    $scope.meta.title.value = response.blogStar.meta.title;

                    var modifiedTimestamp = new Date(response.blogStar.modified.timestamp);
                    ...
                    $scope.modified.user.id.value = response.blogStar.modified.user.id;
                    $scope.modified.user.username.value = response.blogStar.modified.user.username;
                    $scope.name.value = response.blogStar.name;
                    $scope.og.description.value = response.blogStar.og.description;
                    $scope.og.title.value = response.blogStar.og.title;
                    $scope.rate.value = library.numeral.initializeSeparator(response.blogStar.rate, true);
                    $scope.sequence.value = library.numeral.initializeSeparator(response.blogStar.sequence, true);

                    if(response.blogStar.thumbnailList.length > 0) {

                        $scope.thumbnailList.value = response.blogStar.thumbnailList;
                        $scope.thumbnailList.upload.file = response.blogStar.thumbnailList;
                        $scope.thumbnailList.upload.view = true;

                    }

                    $scope.url.value = response.blogStar.url;
                    $scope.view.value = library.numeral.initializeSeparator(response.blogStar.view, true);
                ...
                angular.forEach(response.statusList, function(value, key) {
                    ...
                    if(response.blogStar.id != null) {
                        ...
                        if(value == response.blogStar.status) {
    ...
    $scope.initializePagination = function() {
        ...
        var rest = {
            "data": "",
            "url": document.getElementById("global").getAttribute("data-base-url") + "/blog/star/initialize-pagination"
        };
        global.rest(rest, function(response) {
    ...
    $scope.insert = function(event) {
        ...
        if(valid) {
            ...
            var rest = {
                ...
                "url": document.getElementById("global").getAttribute("data-base-url") + "/blog/star/insert"
            };
            global.rest(rest, function(response) {
            ...
            if(response.result) {
                ...
                if(response.blogStar.id != null) {
                    ...
                    if(response.blogStar.imageList.length > 0) {
                        ...
                        angular.forEach(response.blogStar.imageList, function(value, key) {

                            $scope.imageList.upload.file.push(document.getElementById("global").getAttribute("data-image-url") + "/blog/star/" + value);
                    ...
                    $scope.rate.value = library.numeral.initializeSeparator(response.blogStar.rate, true);
                    $scope.sequence.value = library.numeral.initializeSeparator(response.blogStar.sequence, true);
                    $scope.status.value = $scope.camelCaseToStandardCase(response.blogStar.status);

                    if(response.blogStar.thumbnailList.length > 0) {

                        angular.forEach(response.blogStar.thumbnailList, function(value, key) {

                            $scope.thumbnailList.upload.file.push(document.getElementById("global").getAttribute("data-image-url") + "/blog/star/thumbnail/" + value);
    ...
    $scope.removeFilterPagination = function(event) {
        ...
        var rest = {
            "data": "",
            "url": document.getElementById("global").getAttribute("data-base-url") + "/blog/star/remove-filter-pagination"
        };
        global.rest(rest, function(response) {
    ...
    $scope.setPagination = function(event) {
        ...
        var rest = {
            ...
            "url": document.getElementById("global").getAttribute("data-base-url") + "/blog/star/set-pagination"
        };
        global.rest(rest, function(response) {

            if(response.result) {

                $window.location.href = document.getElementById("global").getAttribute("data-base-url") + "/blog/star/";
    ...
    $scope.uploadImageList = function() {
        ...
        var rest = {
            "data": formData,
            "url": document.getElementById("global").getAttribute("data-base-url") + "/blog/star/upload-image-list"
        };
    ...
    $scope.uploadThumbnailList = function() {
        ...
        var rest = {
            "data": formData,
            "url": document.getElementById("global").getAttribute("data-base-url") + "/blog/star/upload-thumbnail-list"
        };
        global.restMultipart(rest, function(response) {
...
app.directive("blogStarDetail", function() {


    var blogStar = {};


    blogStar.templateUrl = document.getElementById("global").getAttribute("data-base-url") + "/html/blog-star-detail-popup.html";
...
app.directive("blogStarImageListPreview", function() {


    var blogStar = {};


    blogStar.template = "<span ng-repeat=\"data in imageList.upload.file\">" +
        "<img class=\"responsive\" ng-src=\"" + document.getElementById("global").getAttribute("data-image-url") + "/blog/star/{{data}}\" />" +
        "</span>";
...
app.directive("blogStarThumbnailListPreview", function() {


    var blogStar = {};


    blogStar.template = "<span ng-repeat=\"data in thumbnailList.upload.file\">" +
        "<img class=\"responsive\" ng-src=\"" + document.getElementById("global").getAttribute("data-image-url") + "/blog/star/thumbnail/{{data}}\" />" +
        "</span>";
